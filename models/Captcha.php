...
class CaptchaModel extends CI_Model {
	...
	protected $_captcha_key = 'captcha_code_session_%s'; // ==  'captcha_code_session_' . $_session_id;
	 ...
	 * @param   bool    $delete   注册检测时删除掉 异步检测时不删除
	 * @return  bool
	 */
	public function check_word($word , $delete = false) {
		$sessionId  = $this->session->getSessionId();
		$captchaWord = $this->memcache->get( $this->_captcha_key , $sessionId  );
		if($delete){
			$this->memcache->delete( $this->_captcha_key , $sessionId  );		
		}
		
		return strtoupper(trim( $word )) === strtoupper(trim( $captchaWord )) ;
	}
	...
	function record_word($word){
		...
		if( !empty( $word ) ){
			$sessionId  = $this->session->getSessionId();
			$this->memcache->set( $this->_captcha_key, $word , $sessionId  );
