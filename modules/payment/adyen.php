...
class Adyen extends CI_Module {
	...
	public function getPaymentParams($order, $brandCode = 'paypal', $issuerId = '' ){
		$data = array();
		$data['merchantReference'] = $order['order_sn'];
		$data['paymentAmount'] = $order['order_amount']*100;
		$data['currencyCode'] = $order['currency_code'];
		$data['shipBeforeDate'] = date('Y-m-d',strtotime('+3 days'));
		$data['skinCode'] = $this->_getSkinCode();
		$data['merchantAccount'] = self::MERCHANT_ACCOUNT;
		$data['shopperLocale'] = $this->_getLanguageCode($order['language_code']);
		$data['sessionValidity'] = date('c',strtotime('+30 minutes'));
		if($order['pay_country'] != '') $data['countryCode'] = $order['pay_country'];
		$data['shopperEmail'] = $order['email'];
		$data['shopperReference'] = $order['user_id'];
		$data['resURL'] = eb_gen_url('pay/result/'.self::PAY_CODE);
	...
	public function checkPaymentSuccess(){
		...
		if($calculated_sig != $sig) return PS_UNPAID;

		$status = PS_UNPAID;
		switch($result){
			case 'PENDING':$status = PS_PAYING;break;
			case 'AUTHORISED':$status = PS_PAID;break;
			default:$status = PS_UNPAID;break;
		}
